...
class RP_A1:
    ...
    def __init__(self, com="/dev/ttyUSB0", baudrate=115200, timeout=3, rotation=0, scan_type="normal", threaded=True, quality=False):
        self.get_quality = quality
    ...
    def read(self, rotate=False, ):  # It's totally possible to move this to a thread.
        ...
        quality, angles, distances = list(zip(*items))
        if rotate:
            distances, angles = list(zip(*self.rotate_lidar_readings(zip(distances, angles))))
        if self.get_quality:
            return [list(distances), list(angles), list(quality)]
    ...
    def readCartesian(self):
        if self.get_quality:
            distance, angle, quality = np.array(self.getScan())
            angle = np.deg2rad(360 - angle)  # done to un-mirror the lidar, kinda.
            return np.array([distance * np.cos(angle), distance * np.sin(angle), quality]).T
